...
void Game::update(sf::Time elapsedTime)
{
	...
	case GameState::Welcome:
		{
			if (frameNumber % 2 == 1) gameGraphics.update(elapsedTime);
	...
	case GameState::Playing:
		{
			...
			if (remainingTime <= 0) gameState = GameState::GameOver;
			
			if (!gameOver() && raft1.getHealth() > 0)
			{
				if (frameNumber % 4 == 0) raft1.controls = player1.controlRaft(&infoProvider1);
				raft1.update(elapsedTime);
				if (map.fishLocations.empty() || raft1.getHealth() == 0) gameState = GameState::GameOver;
			}

			if (!gameOver() && raft2.getHealth() > 0)
			{
				if (frameNumber % 4 == 2) raft2.controls = player2.controlRaft(&infoProvider2);
				raft2.update(elapsedTime);
				if (map.fishLocations.empty() || raft2.getHealth() == 0) gameState = GameState::GameOver;
			}

			if (!gameOver()) cannonBallsManager.update(elapsedTime);

			if (raft1.getHealth() == 0 || raft2.getHealth() == 0) gameState = GameState::GameOver;
