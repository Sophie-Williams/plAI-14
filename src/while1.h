...
struct while1_MotorControls{
	bool brakeApplied;
	sf::Vector2f thrustVector;
};
struct while1_FishLocation{
	bool caught;
	bool blackListed;
	float blackListTime;
...
public:
	...
	while1_cannon() { timeDelay = (int)(CANNONBALL_FIRE_MIN_LAG*GAME_DESIRED_UPDATE_FPS);
		timeCheck=0; prevVelocity=sf::Vector2f(0,0); onTime=entered=seenOnce=false;}
};
class while1_obstacleMan{
...
class while1 : public Player
{
private:
	//Declare your data structures here
	while1_explorer explore;
	while1_obstacleMan obstacle;
	while1_cannon cannonFire;
	while1_MotorControls movementControls;
	while1_FishLocation fish[NUMBER_OF_FISHES];
public:
