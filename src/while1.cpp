...
#include <stdio.h>
using namespace sf;
int while1_obstacleMan::LHMCountdown=DEFAULT_LHMCountdown;
float while1_obstacleMan::MIN_PING_DIST=RAFT_VISIBILITY_RADIUS*1.0f/1000.0f;
bool while1_obstacleMan::hugsLand=false;
float while1_obstacleMan::OBSTACLEMAN_DISP_ANGLE = PI / 1.5f;
int while1_cannon::gameCounter=1;
Vector2f while1_explorer::explorePoint(InformationProvider *info)
{
...
void while1_explorer::blacklistExplore(InformationProvider *info,Vector2f blacklist)
{
	int i;
	for(i=0;i<8;i++)
	{
...
int while1_explorer::chooseLocation(InformationProvider *info)
{
	int i;
	for(i=0;i<8;i++)
	{
...
void while1_explorer::chooseQueueOrder(InformationProvider *info)
{
...
void while1::motorFunction(sf::Vector2f rFinal,InformationProvider *info, bool needWarpSpeed){
...
void while1_obstacleMan::setStartTime(int t){
	startTime=t;
}

void while1_obstacleMan::resetMode(InformationProvider *info) {
	setStartTime(180 - info->getRemainingGameTime());
	LHMCountdown=DEFAULT_LHMCountdown;
	hugsLand = false;
}

Vector2f while1_obstacleMan::getNextPingPoint(Vector2f v,int n){
	return (1.0f*n*MIN_PING_DIST)*v;
}
int while1_obstacleMan::randDirFixer(){
	srand (time(NULL));
	int random = rand() % 100;
	if(random <= 50) {
		return -1;
...
float while1_obstacleMan::randAngleGen(){
	return OBSTACLEMAN_MAX_ANGLE;
}
Vector2f while1_obstacleMan::dispChange(InformationProvider *info){
...
Vector2f while1_obstacleMan::getObstacleManPosition(Vector2f x,InformationProvider *info){
	/*
...
*/
Vector2f while1_cannon::opponentVelocityVector(Vector2f opponentInitialPosition, InformationProvider *info)
{
...
Vector2f while1_cannon::cannonVector(InformationProvider *info )
{
	//int random,mul;
	float mul;
	...
	else if(info->getOpponentRaftPosition()!=Vector2f (-1,-1))
	{
		...
		//Vector2f velocity;
		prevVelocity = while1_cannon::opponentVelocityVector(prevPosition,info);
		//prevVelocity = velocity;
...
Vector2f while1::flee(InformationProvider *info){
...
Vector2f while1::hide(InformationProvider *info) {
	return info->getRaftPosition();
}

Vector2f while1::enemyPursuit(InformationProvider *info) {
...
void while1::updateDatabase(InformationProvider *info) {
...
Vector2f while1::catchFish(InformationProvider *info){
	//printf("CatchFish\n");
...
int while1::decisionMaker(InformationProvider *info) {
...
void while1::initialise(InformationProvider* info)
{
	obstacle.resetMode(info);
	for(int i=0;i<NUMBER_OF_FISHES;i++){
...
RaftControls while1::controlRaft(InformationProvider* info)
{
...
void while1::getTeamName(char teamName[])
{
	strcpy_s(teamName, 256, "W#!L3-1");
}